...
module Bitcoin
  module SLIP39
    ...
    class SSS
      ...
      # @param [Boolean] legacy_salt if true, build the PBKDF2 salt with the minimum number of bytes for the
      #   identifier. Shares generated by bitcoinrb 1.14.0 and earlier need this to be recovered.
      # @return [String] a master secret.
      def self.recover_secret(shares, passphrase: '', legacy_salt: false)
        ...
        return decrypt(group_shares.values.first, passphrase, exp, id, legacy_salt) if group_threshold == 1
        ...
        decrypt(secret, passphrase, exp, id, legacy_salt)
      end
      ...
      # @param [Boolean] legacy_salt if true, build the salt with the minimum number of bytes for the identifier.
      def self.decrypt(ems, passphrase, exp, id, legacy_salt = false)
        l, r = ems[0...(ems.length / 2)].htb, ems[(ems.length / 2)..-1].htb
        salt = get_salt(id, legacy_salt)
      ...
      # @param [Integer] id id
      # @param [Boolean] legacy if true, encode the identifier with the minimum number of bytes. SLIP-39 requires
      #   two bytes, so a salt built this way only matches shares generated by bitcoinrb 1.14.0 and earlier.
      # @return [String] salt with binary format.
      def self.get_salt(id, legacy = false)
        identifier = legacy ? id.itb : [id].pack('n')
        (Bitcoin::SLIP39::CUSTOMIZATION_STRING.pack('c*') + identifier)
      end
